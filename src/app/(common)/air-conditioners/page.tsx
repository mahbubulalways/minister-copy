...
import getAirConditioner from "@/services/actions/airConditioner/getAirconditioner";
import { TSearchParams } from "@/types";
import Link from "next/link";
import { FaAngleRight } from "react-icons/fa6";
const AirConditionerPage = async ({ searchParams }: TSearchParams) => {
  const query = {
    size: searchParams?.size,
    priceRange: searchParams?.price,
  };
  let products = await getAirConditioner(query);
  if (!products) {
    return <Loader />;
  }
    ...
    <div>
      <div className=" py-5 border-b">
        <div className="container flex items-center gap-2 text-[13px] font-semibold">
          <Link href={"/"}>HOME</Link>
          <FaAngleRight />
          <Link href={"/television"}>Air CONDITIONER</Link>
        </div>
      </div>
