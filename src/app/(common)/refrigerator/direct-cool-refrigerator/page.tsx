...
import getDirectCoolRefrigerator from "@/services/actions/refrigerator/getDirectCoolRefrigerator";
import { TSearchParams } from "@/types";
import Link from "next/link";
import { FaAngleRight } from "react-icons/fa";

const DirectCoolRefrigerator = async ({ searchParams }: TSearchParams) => {
  const query = {
    size: searchParams?.size,
    priceRange: searchParams?.price,
  };
  const products = await getDirectCoolRefrigerator(query);
  if (!products) {
    return <Loader />;
  }
...
export default DirectCoolRefrigerator;
