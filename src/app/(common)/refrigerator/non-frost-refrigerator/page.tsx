...
import getNonFrostRefrigerator from "@/services/actions/refrigerator/getNonFrostRefrigerator";
import { TSearchParams } from "@/types";
import Link from "next/link";
import { FaAngleRight } from "react-icons/fa";

const NonFrostRefrigerator = async ({ searchParams }: TSearchParams) => {
  const query = {
    size: searchParams?.size,
    priceRange: searchParams?.price,
  };
  const products = await getNonFrostRefrigerator(query);
  if (!products) {
    return <Loader />;
  }
...
export default NonFrostRefrigerator;
