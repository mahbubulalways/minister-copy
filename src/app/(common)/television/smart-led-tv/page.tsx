...
import getSmartLedTelevisions from "@/services/actions/television/getSmartLedTelevision";
import { TSearchParams } from "@/types";
import Link from "next/link";
import React from "react";
import { FaAngleRight } from "react-icons/fa";

const SmartLedTVPage = async ({ searchParams }: TSearchParams) => {
  const query = {
    size: searchParams?.size,
    priceRange: searchParams?.price,
  };
  const products = await getSmartLedTelevisions(query);
  if (!products) {
    return <Loader />;
  }
    ...
    <div>
      <div className="py-5 border-b">
        <div className="container flex items-center gap-2 text-[13px] font-semibold">
          ...
          <FaAngleRight />
          <Link href={"/television/smart-led-tv"}>SMART LED TV</Link>
        </div>
      </div>
